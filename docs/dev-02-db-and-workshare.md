...
## 4-1. B3 칩 라벨과 재료 이름 (마이그레이션 004)

B3 온보딩 칩은 `우유·유제품`, `밀·빵` 처럼 **묶음 이름**이다. 재료 이름이 아니다.

이걸 `ingredients` 에 그대로 넣으면 `우유` 와 `우유·유제품` 이 따로 남아
**FODMAP 값이 없는 껍데기에 연결된다. 계산이 조용히 틀린다.**

→ 칩 라벨은 `ingredients.aliases` 에 넣는다. 온보딩은 이름과 별칭 둘 다로 찾는다.

**새 칩을 추가할 때는 반드시 별칭도 같이 등록할 것.**
