...
### `GET /onboarding/safety/questions` — B1 화면 진입 시
...
{ "title": "해당되는 것이 있나요?",
  ...
  "items": [
    ...
    { "key": "night_symptoms", "label": "자다가 증상 때문에 깰 때가 있어요" },
    { "key": "fever",          "label": "열이 계속 나요" },
    { "key": "vomiting",       "label": "구토를 반복해요" },
    { "key": "anemia",         "label": "빈혈 진단을 받은 적이 있어요" },
    { "key": "onset_after_50", "label": "50세 이후에 증상이 처음 시작됐어요" }
  ],
  "exception": { "key": "cleared_by_doctor", "label": "이미 병원에서 확인했어요" },
  "none_label": "해당 없어요" }
```

**항목을 하드코딩하지 마세요.** 안전 항목이라 나중에 늘어날 가능성이 높고,
그때 앱을 새로 배포하지 않아도 되게 하려는 것입니다.

`exception` 은 **뭔가 하나라도 체크했을 때만** 보여줍니다 (spec-04 V1-213).

### `POST /onboarding/safety` — B1 안전 확인

```json
// 요청 — questions 의 key 를 그대로 씁니다
{ "blood_in_stool": false, "weight_loss": false, "night_symptoms": false,
  "fever": true, "vomiting": false, "anemia": false, "onset_after_50": false,
...
`blocked: true` 면 프론트는 **B1x 로 보내고 온보딩을 끝냅니다.**
**B1x 의 문구도 하드코딩하지 마세요.** `title` / `body` / `footer` 를 그대로 씁니다.

`cleared_by_doctor: true` 면 차단되지 않습니다. 다만 `flags` 는 채워져 서버에 기록됩니다.
...
`avoided_foods` 는 그대로 **나의 식탁의 "다시 먹어볼 음식"(`to_try`)** 이 됩니다.
G 화면에는 "피하는 음식" 구획이 없습니다 — 3구획뿐이고, 아직 확인해본 적 없는 것들이니 여기가 맞습니다.

**알레르기와 셀리악(밀)은 여기서 빠집니다.** 나의 식탁에 들어오면 언젠가 도전 대상이 되는데,
알레르기는 양과 무관하게 위험해서 제안 자체가 있으면 안 됩니다 (SF-02).
그래서 `avoided_foods` 에 "우유"를 넣어도 `allergies` 에도 있으면 식탁에 안 들어옵니다.
...
기록이 없는 신규 사용자는 `cards` 가 빈 배열입니다 — **빈 화면 디자인이 필요합니다.**

`type` 종류

| type | 언제 | 특징 |
|---|---|---|
| `challenge_progress` | 진행 중인 도전이 있을 때 | `action` 으로 F3 복귀. **이게 있으면 제안은 안 옵니다** |
| `challenge_suggestion` | 권할 게 있을 때 | `action` = 시작할게요 / `dismiss` = 나중에 |
...
```
POST /home/cards/suggestion/snooze
{ "ingredient_id": 12 }   →   { "ok": true, "days": 14 }
```

거절이 아니라 **미루기**입니다. 2주 뒤 다시 제안됩니다.
누른 직후에는 `/home` 을 다시 불러 카드를 갱신하세요 — 다른 재료로 바뀝니다.
...
그때 `notice.title` / `notice.body` / `notice.footer` 를 띄우세요.
**온보딩에서 걸렸을 때와 문장이 다릅니다.** 같은 걸로 합치지 마세요 (SF-03 상시 감시).
...
**제안할 게 없으면 `204 No Content` 입니다.** 다음 경우 전부 204 입니다.

| 상황 | 이유 |
|---|---|
| 진행 중인 도전이 있음 | 한 번에 하나만 |
| 확인된 후보가 3개 쌓임 | 하드 스톱 (spec-04 V1-652) |
| 첫 도전으로부터 3개월 경과 | 하드 스톱 |
| SF-03 경고 신호 감지 중 | 안전 |
| 더 확인할 항목이 없음 | — |

알레르기·셀리악 항목은 **절대 제안에 나오지 않습니다.**
...
| grade | 언제 | 저장하면 |
|---|---|---|
| `tolerated` | 3회 중 반응 0~1회 | 안심하고 먹는 음식 |
| `reduce_amount` | 3회 중 반응 2~3회 (2-of-3 재현) | 확인된 후보 |
| `recheck_later` | 건너뛴 회차가 있음 | 다시 먹어볼 음식 (그대로) |

반응 1회는 재현이라고 부르지 않습니다. 한 번의 반응은 그날의 수면·스트레스·과식과 구분되지 않습니다.
...
****빈 구획은 `sections` 에서 아예 빠집니다.** 개수가 3개라고 가정하지 마세요.
온보딩 직후에는 "다시 먹어볼 음식" 하나만 옵니다.
...
| 값 | 의미 |
|---|---|
| `red_flag` | 혈변·체중감소 등 키워드. **답변 LLM 을 부르지도 않았습니다** |
| `medical_question` | 의료 질문으로 분류됨 |
| `low_confidence` | 분류가 애매함 → 보수적으로 막음 |
| `output_rejected` | 답변이 금지 패턴에 걸려 2회 재시도 후 폴백 |

**응답이 느립니다.** 분류 + 답변 두 번 부르느라 보통 2초 안팎입니다.
입력창에 로딩 상태를 반드시 넣어주세요. 그 사이 중복 전송은 막아주세요.
