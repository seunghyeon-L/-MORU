...
FOODS = [
    ...
    ("피자",       300, False),
    ("샐러드",     200, False),
    # 대체 메뉴(H5)로 제안하는 것들. 마스터에 없으면 사용자가 H5 에서
    # "기록하기" 를 눌렀을 때 LLM 추정으로 떨어져 재료가 얕게 잡힌다.
    ("맑은 순두부탕",      400, True),
    ("간장 돼지고기 덮밥", 400, False),
    ("두부 간장 덮밥",     400, False),
    ("오징어 숙주볶음",    300, False),
    ("간장떡볶이",        350, False),
    ("떡국",             500, True),
...
RECIPES = {
    ...
    # 부대찌개는 FOODS 에만 있고 레시피가 빠져 있었다.
    # 재료가 없으면 6축이 0 으로 계산되고 D2 체크리스트도 빈다.
    "부대찌개":   [("라면", 60, False), ("소시지", 80, False), ("김치", 60, False),
                   ("양파", 50, True), ("두부", 60, False), ("대파", 15, True),
                   ("마늘", 8, True), ("고춧가루", 6, True)],

    # ── 대체 메뉴 (H5) ──
    # 그램은 원본보다 FODMAP 이 낮게 나오도록 잡았다.
    # 양파·마늘이 들어가면 금방 뒤집힌다 (마늘 12.5g/100g).
    # "맑은"·"간장" 계열이라 마늘을 적게 잡거나 뺐다.
    "맑은 순두부탕":      [("두부", 200, False), ("대파", 10, True),
                          ("마늘", 4, True), ("계란", 50, False)],
    "간장 돼지고기 덮밥": [("돼지고기", 150, False), ("쌀", 210, False),
                          ("간장", 20, False), ("양파", 40, False),
                          ("설탕", 8, False), ("대파", 10, False)],
    "두부 간장 덮밥":     [("두부", 150, False), ("쌀", 210, False),
                          ("간장", 20, False), ("대파", 10, False),
                          ("참기름", 5, False)],
    "오징어 숙주볶음":    [("오징어", 150, False), ("숙주", 100, False),
                          ("대파", 15, False), ("간장", 15, False),
                          ("참기름", 5, False)],
    "간장떡볶이":         [("떡", 200, False), ("어묵", 60, False),
                          ("간장", 20, False), ("설탕", 10, False),
                          ("대파", 15, False)],
    "떡국":              [("떡", 180, False), ("계란", 50, False),
                          ("대파", 10, True), ("간장", 10, True)],
    "쌀국수":            [("쌀", 150, False), ("숙주", 50, False),
                          ("대파", 10, True), ("간장", 10, True)],
    "계란찜":            [("계란", 120, False), ("대파", 5, False)],
    "소고기 무국":        [("소고기", 100, False), ("무", 120, False),
                          ("대파", 10, True), ("간장", 10, True)],
    "계란볶음밥":         [("쌀", 210, False), ("계란", 60, False),
                          ("당근", 20, False), ("참기름", 5, False)],
    "닭가슴살 구이":      [("닭고기", 180, False), ("올리브유", 8, False),
                          ("소금", 2, False)],
}
